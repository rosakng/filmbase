...
def _inverse_squaring_helper(T0, theta):
    """
    A utils function for inverse scaling and squaring for Pade approximation.
...
def _fractional_power_pade_constant(i, t):
    # A utils function for matrix fractional power.
    if i < 1:
...
def _remainder_matrix_power_triu(T, t):
    ...
    if np.array_equal(T0, np.diag(T0_diag)):
        ...
        # returned by the inverse squaring utils.
